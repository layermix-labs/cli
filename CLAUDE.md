...
CLI subcommands (see `src/cli/index.tsx`): `list`, `validate`, `init`, `run [taskIds...]` (default — `my-runner [args]` is the same as `my-runner run [args]`). `run` flags: `-t <tag>`, `--dry-run-json`, `--concurrency <n>`, `--output-only-failed`, `--no-tui`, `--ci`, `--junit <path>`. `init` also accepts `--force` to overwrite an existing `task-runner.json`. The previous `interactive` (checkbox selection) command was removed; there is no checkbox UI anymore — pass task ids or `-t <tag>` directly.
...
4. **Executor** (`src/core/executor.ts`) — `EventEmitter`
   ...
   - CI detection: when the `is-ci` package detects a CI env (checks `CI`, `CONTINUOUS_INTEGRATION`, `GITHUB_ACTIONS`, etc.) or `--ci` is set, `ciMode = true` forces linear mode (TUI is suppressed even on a TTY). `useTui = isTTY && !--no-tui && !ciMode`. Exit is still 1 on failure.

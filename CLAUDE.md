...
- Run CLI: `npm start -- [args]` (e.g. `npm start`, `npm start -- build --ci`, `npm start -- -t test --dry-run-json`, `npm start -- list`, `npm start -- init`). `run` is the default subcommand, so the `run` keyword can be omitted. Bare `npm start` with no task ids or `-t <tag>`: TUI opens idle, piped non-CI linear mode prints a yellow hint and exits 0, `--ci`/`--ai`/CI-detected exits **1** with a stderr error. Unknown task ids or tags also exit 1 in every mode (silent no-op would mask typos). Set `defaultRun` in `task-runner.json` to give an empty invocation a target; without it the CI/AI path must get an explicit target or it fails. The previous behavior — CI/AI mode auto-running everything on empty target — was removed because `layermix --ai` in an unfamiliar repo would silently execute the entire pipeline; the current behavior goes one step further and makes empty target in CI/AI a hard error, so a scheduled agent or CI job can't silently exit green.
...
4. **Executor** (`src/core/executor.ts`) — `EventEmitter`
   ...
   - CI / AI-agent detection: `ciMode` is true when any of these match — `is-ci` detects a CI env (checks `CI`, `CONTINUOUS_INTEGRATION`, `GITHUB_ACTIONS`, etc.); `--ci` flag is set; `isAiAgent()` detects a coding-agent env (`CLAUDECODE`, `CLAUDE_CODE_ENTRYPOINT`, `CURSOR_AGENT`, `CURSOR_TRACE_ID`, `AIDER_MODEL`, `AIDER_CHAT_HISTORY_FILE`, `CONTINUE_SESSION_ID`, or the generic opt-in `AI_AGENT`); or `--ai` flag is set. `--ai` is just an alias for `--ci` — both force linear mode. `useTui = isTTY && !ciMode`. Exit-code rules by target state: unknown task id or tag → exit 1 in every mode (validated in `validateExplicitTarget` right after `defaultRun` resolution); empty target in CI/AI → exit 1 (stderr error); empty target in piped non-CI linear → exit 0 (yellow hint on stdout); empty target in TUI → idle, no exit. `defaultRun` is the only way to give an empty invocation a target; it kicks in equally in CI/AI and piped-non-CI linear modes (just not in TUI, which stays idle to preserve the explicit "pick what to run" UX). The previous "CI/AI mode auto-runs all tasks on empty target" behavior was removed because `layermix --ai` in an unfamiliar repo used to silently execute the whole pipeline; the follow-up "empty target in CI/AI is an error, not a soft hint" prevents the same invocation from silently exiting 0 with nothing done.

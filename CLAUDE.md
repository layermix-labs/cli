...
4. **Executor** (`src/core/executor.ts`) — `EventEmitter`
   ...
   - CI / AI-agent detection: `ciMode` is true when any of these match — `is-ci` detects a CI env (checks `CI`, `CONTINUOUS_INTEGRATION`, `GITHUB_ACTIONS`, etc.); `--ci` flag is set; `isAiAgent()` detects a coding-agent env (`CLAUDECODE`, `CLAUDE_CODE_ENTRYPOINT`, `CURSOR_AGENT`, `CURSOR_TRACE_ID`, `AIDER_MODEL`, `AIDER_CHAT_HISTORY_FILE`, `CONTINUE_SESSION_ID`, or the generic opt-in `AI_AGENT`); or `--ai` flag is set. `--ai` is just an alias for `--ci` — both produce the same linear-mode, auto-run-all-on-empty-target behavior. `useTui = isTTY && !ciMode`. Exit is still 1 on failure. Bare `run` with no task ids or tag is only auto-expanded to "all tasks" in CI/AI mode — in the non-CI case (TUI or piped linear) it's treated as "no target" and nothing is executed (TUI stays idle; linear prints a hint and exits 0). This gates on `ciMode`, not on TUI vs linear, so piped stdout without `--ci`/`--ai` and without a detected CI/AI env still respects the non-CI behavior.

...
const Overview: React.FC<OverviewProps> = ({ tasks, width }) => {
  ...
  const totalDuration = useMemo(() => {
      const starts = taskList.map(t => t.startTime).filter((t): t is number => t !== undefined);
      const ends = taskList.map(t => t.endTime || (t.status === 'RUNNING' ? now : t.startTime)); 
  ...
  const stats = useMemo(() => {
      ...
      const pending = taskList.filter(t => t.status === 'IDLE' || t.status === 'QUEUED').length;
  ...
  const chartWidth = Math.max(5, innerWidth - LABEL_WIDTH - DURATION_GAP - DURATION_WIDTH);
  ...
  const renderBar = (task: TaskState) => {
      ...
      let offsetChars = Math.min(chartWidth - 1, Math.max(0, Math.floor(offsetPct * chartWidth)));
  ...
  const bottleneck = useMemo(() => {
      const sorted = [...taskList].sort((a, b) => (b.duration || 0) - (a.duration || 0));
      return sorted[0];
  }, [taskList]);
         ...
         {taskList.map(task => (
             <Box key={task.id} minHeight={1} width={innerWidth}>
                 <Box width={LABEL_WIDTH} flexShrink={0}>
                     <Text wrap="truncate-end">{task.id.length > LABEL_WIDTH - 2 ? task.id.slice(0, LABEL_WIDTH - 4) + '...' : task.id}</Text>
                 </Box>
                 <Box width={chartWidth} flexShrink={0} overflowX="hidden">
                     {renderBar(task)}
                 </Box>
                 <Box width={DURATION_WIDTH} marginLeft={DURATION_GAP} flexShrink={0}>
                     <Text dimColor wrap="truncate-end">{task.duration ? `${(task.duration/1000).toFixed(1)}s` : (task.status === 'RUNNING' ? '...' : '-')}</Text>
                 </Box>
             </Box>
         ))}
      </Box>

      <Box marginTop={1} borderStyle="single" borderColor="gray" flexDirection="column" width={innerWidth}>

...
const TagDetail: React.FC<TagDetailProps> = ({ tag, taskIds, tasks, width }) => {
  return (
    <Box
    ...
    >
      <Box marginBottom={1}>
        ...
        <Text dimColor> ({taskIds.length} task{taskIds.length === 1 ? '' : 's'})</Text>
      </Box>
      ...
      <Box marginTop={1}>
        ...
        <Text dimColor> to run these tasks (with their upstream dependencies).</Text>
      </Box>
    </Box>
  );
};

...
describe('CLI e2e', () => {
  ...
  it('run with CI=true emits structured JSON report', async () => {
    ...
    expect(res.exitCode).toBe(1);
    const match = res.stdout.match(/---BEGIN MY-RUNNER-REPORT---\n([\s\S]+?)\n---END MY-RUNNER-REPORT---/);
  ...
  it('--ci flag alone implies --no-tui and emits the report', async () => {
    ...
    const res = await execa('node', [VITE_NODE, '--root', ROOT, CLI, 'run', '--ci'], {
      cwd: failing,
      reject: false,
      env: {
        ...process.env,
  ...
  it('init writes a task-runner.json with $schema reference', async () => {
    ...
    try {
      ...
      expect(res.exitCode).toBe(0);
      const written = JSON.parse(fs.readFileSync(path.join(tmp, 'task-runner.json'), 'utf8'));
